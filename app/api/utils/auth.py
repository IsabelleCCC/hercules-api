...
class Auth:
    ...
    def create_access_token(self, email: Union[str, Any], name: Union[str, Any], id: Union[int, Any], expires_delta: int = None) -> str:
        if expires_delta is not None:
            expires_delta = datetime.utcnow() + expires_delta
        else:
            expires_delta = datetime.utcnow() + timedelta(minutes=self.ACCESS_TOKEN_EXPIRE_MINUTES)

        to_encode = {"exp": expires_delta, "email": str(email), "name":str(name), "id":int(id)}
